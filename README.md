...
# Usage
1. Run `run.bat` to start the app on your browser. Or run `run_chrome.bat` for only Google Chrome with independent window mode (without menu bar, status bar and so on). And also run `run_chrome.bat` for only Vivaldi. If you need, run `run_nocapture.bat` to start the app without capture functionality.
